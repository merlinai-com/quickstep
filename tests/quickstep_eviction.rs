...
#[test]
fn second_chance_clears_hot_pages_before_eviction() {
    debug::reset_debug_counters();
    let db = new_small_cache_db();
    let payload = vec![0u8; 1024];
    ...
    let passes_before = debug::second_chance_passes();

    {
        let mut tx = db.tx();
        for i in 256..320 {
            let key = format!("key-{i:04}");
            tx.put(key.as_bytes(), &payload).expect("insert more");
        }
        tx.commit();
    }

    assert!(
        debug::second_chance_passes() > passes_before,
        "second-chance path should run before eviction"
    );
}

...
| 1.3 | Implement split/merge logic | WIP | Root-level leaf splits + cascading parent splits implemented with debug instrumentation; new siblings publish map-table identities immediately; baseline cache eviction now flushes dirty leaves on demand; leaf merges (plan/apply, parent rewiring, root demotion) and debug helpers are in place, while delete-triggered merge thresholds + second-chance eviction remain pending |
